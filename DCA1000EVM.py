...
class DCA1000EVM:
    ...
    def fastRead_in_Cpp(self, numframes=1):
        ...
        receivedData, firstPacketNum, receivedPacketNum = self.postProcPacket(recvQueue, packetNum)
        databuf = receivedData[0:numframes * UINT16_IN_FRAME]
        print("received packet num:%d,expected packet num:%d \n" % (len(receivedPacketNum), packetNum))

        return databuf

    def get_frame(self, isMIMO=False, isBeamforming=False, timeout=1):
        raw_frame = self.read(timeout)
        ...
        raw_frame = np.reshape(raw_frame, (ADC_PARAMS['chirps'], ADC_PARAMS['tx'],
                                           ADC_PARAMS['rx'], ADC_PARAMS['samples'] // 2, ADC_PARAMS['IQ'], 2))

        raw_frame = np.transpose(raw_frame, (0, 1, 2, 3, 5, 4))
        raw_frame = np.reshape(raw_frame, (ADC_PARAMS['chirps'], ADC_PARAMS['tx'],
                                           ADC_PARAMS['rx'], ADC_PARAMS['samples'], ADC_PARAMS['IQ']))

        # [Chirps, TX, RX, Samples]
        raw_frame = (1j * raw_frame[:, :, :, :, 0] + raw_frame[:, :, :, :, 1]).astype(np.complex64)

        if isMIMO:
            # [Chirps * VirtualAnts * Samples]
            raw_frame = np.reshape(raw_frame[:, 0:ADC_PARAMS['tx'], :, :],
                                  (ADC_PARAMS['chirps'], ADC_PARAMS['tx'] * ADC_PARAMS['rx'], ADC_PARAMS['samples']))

        if isBeamforming:
            raw_frame = np.mean(raw_frame, axis=1)
        return raw_frame



    def write_frames_to_file(self, filename="raw_data.bin", numframes=1):
        # more frame also can be saved
        if 1:
            data_buf = self.read()
            ...
            '''Use third_part udp, collect few frames, more frame may be error to save file.'''
    ...
    def _read_data_packet(self, timeout=1):
        ...
        ready = select.select([self.data_socket], [], [], timeout)
        if ready[0]:
            # print("Ready value:{}\n".format(ready))
            data, addr = self.data_socket.recvfrom(MAX_BYTES_PER_PACKET)  # MAX_PACKET_SIZE
            packet_num = struct.unpack('<1l', data[:4])[0]
            ...
            raise Exception("UDP Socket Recv Data Error...[ Unsolved .... sad....]")
    ...
    def parser_raw_adc_data(raw_bin_file, config_file_name, isMIMO=False, isBeamforming=False):
        ...
        """
        _, _, ADC_PARAMS, _ = DCA1000EVM.read_config(config_file_name)
    ...
    def configure(self, FPGAjsonFilePath, IWRcfgFilePath):
        ...
        """
        print("\n DCA1000EVM FPGA Configuration Start...")
        ...
        print(ADC_PARAMS_l)
        print(CFG_PARAMS_l)
        print("LVDSDataSizePerChirp: {}, must <= maxSendBytesPerChirp: {}".format(LVDSDataSizePerChirp_l,
                                                                                  maxSendBytesPerChirp_l))
        print("System connection check: {}\n".format(self.sys_alive_check()))
        print("FPGA Version: {}\n".format(self.read_fpga_version()))
        print("Config fpga: {}\n".format(self.config_fpga(FPGAjsonFilePath)))
        print("Config record packet delay: {}\n".format(self.config_record(FPGAjsonFilePath)))
        print("DCA1000EVM FPGA Configuration Stop...\n")
